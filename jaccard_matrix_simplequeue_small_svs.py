...
parser.add_argument("-s", "--winSize", help="Window size", type=int, action = "store", default = 1)
parser.add_argument("-m", "--maxLength", help="Maximum distance between pairs of windows being compared", type=int, action = "store", default = 1)
...
#how many windows are we comparing?
decResult=args.maxLength/args.winSize
num_win=round(decResult+0.5)
...
def freqs_wrapper(inQueue, resultQueue, number_win, inFile):
    while True:
        ...
        outIndex=0
        for index2, row2 in windowFile.iloc[windowNumber:windomMax,:].iterrows():
